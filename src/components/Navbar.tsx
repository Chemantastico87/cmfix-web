...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-brand-dark/90 backdrop-blur-md border-b border-brand-border/60 transition-colors">
          ...
          <div className="pt-4 border-t border-brand-border/60 flex flex-col gap-2.5">
            ...
            {user ? (
              ...
              <Link
                to="/login"
                onClick={() => setIsOpen(false)}
                className="flex items-center justify-center gap-2 w-full py-2.5 px-4 rounded-lg text-sm font-medium text-slate-400 hover:text-slate-200"
              >
                <ShieldCheck className="w-4 h-4" />
                <span>Acceso Profesionales CM FIX</span>
              </Link>
            )}
          </div>
        </div>
